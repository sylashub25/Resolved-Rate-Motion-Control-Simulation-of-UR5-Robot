function [qp, xerror, xp]=posicion_ur5_R1(t,x)
disp(t);
[~, n]=size(t);
%Estados
q1=x(1);
...
dy = gradient(yd);
dz = gradient(zd);
z_axis = [dx; dy; dz];  % dirección de avance

%% Ciclo for para obtener el la orientacion deseada en modo serpiente elegante
R_d_all = zeros(3,3,n);
wtd = zeros(3,n);  % ángulos de Euler deseados

for i = 1:n
    z = z_axis(:,i) / norm(z_axis(:,i));  % normaliza

    x_ref = [0; 0; 1];  % referencia para eje X
    x = x_ref - dot(x_ref, z)*z;
    x = x / norm(x);

    y = cross(z, x);  % completa el marco

    R_d = [x y z];    % matriz de rotación deseada
    R_d_all(:,:,i) = R_d; %Guardamos todas las orientaciones

    wtd(:,i) = EULERXYZINV1(R_d);  % orientación deseada en Euler XYZ y guardamos en wtd
end

%% Verificamos que la oreintacion sea fisicamente realizable por el robot
for i = 1:n
    if any(isnan(wtd(:,i))) || any(isinf(wtd(:,i)))
        fprintf('Paso %d: Orientacion no valida',i);
    end

    R_d = eul2rotm(wtd(:,i)', 'XYZ');
    if norm(R_d'*R_d - eye(3)) > 1e-3 || abs(det(R_d) - 1) > 1e-3
        warning("Paso %d: R_d no ortonormal", i);
    end
end
...
%% Posicion y orientacion actuales obtenidos con ayuda de la cinematica directa
%Cinemática directa para conocer la posición actual
Ha=ur5fwdtrans(q, 6);
%Posiciones actuales en el espacio cartesiano
PosicionActual=Ha(1:3,4); %Posicion actual
%Orientaciones actuales en el espacio cartesiano
wa=EULERXYZINV1(Ha(1:3,1:3));  %Orientacion actual
